<script lang="ts">
  ...
  import { hydrateLearningBackendSnapshot, learningState, restoreLearningSnapshot, saveLearningSnapshot } from '$state/learning-state.svelte';
  ...
  let progressHydratedForUserId = $state('');
  ...
  $effect(() => {
    if (!betaSession.ready || !betaSession.user?.id) {
      progressHydratedForUserId = '';
      return;
    }

    if (progressHydratedForUserId === betaSession.user.id) return;
    progressHydratedForUserId = betaSession.user.id;
    void hydrateLearningBackendSnapshot();
  });
